...
export default function Navbar({ currentPath }) {
  ...
  const isActive = (path) =>
    currentPath === path ? "font-bold scale-110 text-[#102F17]" : "";
  ...
  return (
    <header className="bg-[#F1F1F1] shadow-md">
      <div className="max-w-screen-xl mx-auto px-6 py-3 flex items-center justify-between">
        ...
        {/* Desktop Navbar (Hidden Below 950px) */}
        <nav className="hidden navbar-md:flex font-medium text-xl space-x-8 text-[#102F17]">
          <Link
            href="/"
          ...
          <Link
            ...
            onClick={(e) => {
              e.preventDefault();
              scrollToSectionDesktop("services");
            }}
            className={`transition-all hover:font-bold hover:scale-110 ${isActive(
              "/services"
            )}`}
          ...
          <Link
            href="/sustain360"
            className={`font-bold text-red-500 flex items-center gap-1 ${isActive(
              "/sustain360"
            )}`}
          >
            Sustain360
        ...
        {/* Work With Us Button (Hidden Below 950px) */}
        {!isSidebarOpen && (
          <div className="hidden navbar-md:flex">
        ...
        {/* Hamburger Button (Visible Below 950px) */}
        {!isSidebarOpen && (
          <button
      ...
      {isSidebarOpen && (
        <motion.div
          className="fixed inset-0 z-50 navbar-md:hidden"
